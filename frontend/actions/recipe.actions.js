      ...
      if (searchResponse.ok) {
         ...
         if (searchData.data && searchData.data.length > 0) {
           ...
           const savedRecipeResponse = await fetch(
             `${STRAPI_URL}/api/saved-recipes?filters[user][id][$eq]=${user.id}&filters[recipe][id][$eq]=
...
// Remove recipe form user's collection (unbookmark)
...
export async function getSavedRecipes() {
  try {
    ...
    const response = await fetch(
      `${STRAPI_URL}/api/saved-recipes?filters[user][id][$eq]=${user.id}&populate[recipe][populate]=*&
      sort=savedAt:desc`,
      {
        headers: {
          Authorization: `Bearer ${STRAPI_API_TOKEN}`,
